...
public class ScheduledAnnotationBeanPostProcessor
     ...
     */
    public ScheduledAnnotationBeanPostProcessor(PrometheusMeterRegistry prometheusMeterRegistry) {
        this.registrar = new ScheduledTaskRegistrar();
        this.prometheusMeterRegistry = prometheusMeterRegistry;
        this.counter = Counter.build().name("task_exec_count").help("任务执行次数统计").labelNames("name")
                .register(this.prometheusMeterRegistry.getPrometheusRegistry());
    }
     ...
     * @since 5.1
     */
    public ScheduledAnnotationBeanPostProcessor(ScheduledTaskRegistrar registrar, PrometheusMeterRegistry prometheusMeterRegistry) {
    ...
    private void finishRegistration() {
        ...
        if (this.beanFactory instanceof ListableBeanFactory) {
            ...
            //调用接口方法configureTasks来向registrar中心注入该bean，这样就实现了将定时任务交给springboot来管理，并且可以自己定义corn来控制定时任务的执行时间以及是否执行
        ...
        if (this.registrar.hasTasks() && this.registrar.getScheduler() == null) {
            ...
            try {
                ...
                // 开始注入 ScheduledExecutorService bean
                if (logger.isTraceEnabled()) {
        ...
        //将不同方式注册的定时任务统一放到scheduledTasks中管理
    ...
    //所有的bean初始化完成后，开始调用该方法将每个bean以及它的名称传入该方法
    @Override
    public Object postProcessAfterInitialization(Object bean, String beanName) {
        ...
        if (!this.nonAnnotatedClasses.contains(targetClass) &&
            ...
            if (annotatedMethods.isEmpty()) {
                //没有就加入到nonAnnotatedClasses中，下次不执行
                ...
                // 有就将方法加入到定时任务，processScheduled创建线程加入到taskScheduler中
    ...
    protected void processScheduled(Scheduled scheduled, Method method, Object bean) {
        try {
            ...
            if (StringUtils.hasText(initialDelayString)) {
                logger.info("bean name:" + bean.toString() + " initalDelay:" + initialDelay + " initialDelayString:" + initialDelayString);
    ...
    protected Runnable createRunnable(Object target, Method method) {
        ...
        return new ScheduledMethodRunnable(target, invocableMethod,this.counter);
    }
