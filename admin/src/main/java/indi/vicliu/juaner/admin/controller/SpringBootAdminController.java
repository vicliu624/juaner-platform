...
public class SpringBootAdminController {
    ...
    @GetMapping(path = "/instances")
    public Result instances(@RequestParam("name") String name) {
        List<Instance> instances = new ArrayList<>();
    ...
    public Result instant(@RequestParam("id") String id) {
        List<Instance> instances = new ArrayList<>();
        Flux<Instance> instanceFlux = instanceRegistry.getInstances(id).filter(Instance::isRegistered);
        instanceFlux.subscribe(instances::add);
        log.info("size:{}",instances.size());
